...
function initializeNewSession() {
    ...
    conversationHistory = [];
    console.log('New Gemini conversation session started:', currentSessionId);
}
...
function saveConversationTurn(transcription, aiResponse) {
    ...
    console.log('Saved Gemini conversation turn:', conversationTurn);
...
async function initializeGeminiSession(apiKey, customPrompt = '', profile = 'interview', language = 'en-US', isReconnection = false) {
    if (isInitializingSession) {
        console.log('Gemini session initialization already in progress');
        return false;
    }
...
function setupGeminiIpcHandlers(geminiSessionRef, prefix = 'gemini') {
    // Store the geminiSessionRef globally for reconnection access
    global.geminiSessionRef = geminiSessionRef;

    ipcMain.handle(`${prefix}-initialize-session`, async (event, apiKey, customPrompt, profile = 'interview', language = 'en-US') => {
    ...
    ipcMain.handle(`${prefix}-send-audio-content`, async (event, { data, mimeType }) => {
    ...
    ipcMain.handle(`${prefix}-send-mic-audio-content`, async (event, { data, mimeType }) => {
        if (!geminiSessionRef.current) return { success: false, error: 'No active Gemini session' };
        try {
            process.stdout.write(',');
            await geminiSessionRef.current.sendRealtimeInput({
    ...
    ipcMain.handle(`${prefix}-send-image-content`, async (event, { data, debug }) => {
        if (!geminiSessionRef.current) return { success: false, error: 'No active Gemini session' };
    ...
    ipcMain.handle(`${prefix}-get-current-session`, async event => {
        try {
            ...
            console.error('Error getting current Gemini session:', error);
            return { success: false, error: error.message };
        }
    });
    ...
    ipcMain.handle(`${prefix}-start-new-session`, async event => {
        try {
            ...
            console.error('Error starting new Gemini session:', error);
            return { success: false, error: error.message };
        }
    });
    ...
    console.log(`Gemini IPC handlers initialized with prefix: ${prefix}`);
}

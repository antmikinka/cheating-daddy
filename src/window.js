...
function createWindow(sendToRenderer, sessionRefs, randomNames = null) {
        ...
        setTimeout(() => {
            ...
            `
                ...
                .then(async savedSettings => {
                    ...
                    updateGlobalShortcuts(keybinds, mainWindow, sendToRenderer, sessionRefs);
                })
                .catch(() => {
    ...
    setupWindowIpcHandlers(mainWindow, sendToRenderer, sessionRefs);
...
function updateGlobalShortcuts(keybinds, mainWindow, sendToRenderer, sessionRefs) {
    ...
    // Register emergency erase shortcut - updated to handle both session types
    if (keybinds.emergencyErase) {
        try {
            globalShortcut.register(keybinds.emergencyErase, async () => {
                console.log('Emergency Erase triggered!');
                if (mainWindow && !mainWindow.isDestroyed()) {
                    mainWindow.hide();

                    // Clean up both session types
                    if (typeof sessionRefs === 'function') {
                        const refs = sessionRefs();
                        if (refs.gemini && refs.gemini.current) {
                            try {
                                refs.gemini.current.close();
                                refs.gemini.current = null;
                            } catch (error) {
                                console.warn('Error closing Gemini session during emergency erase:', error);
                            }
                        }
                        if (refs.openrouter && refs.openrouter.current) {
                            try {
                                refs.openrouter.current = null;
                            } catch (error) {
                                console.warn('Error closing OpenRouter session during emergency erase:', error);
                            }
                        }
                    }
...
function setupWindowIpcHandlers(mainWindow, sendToRenderer, sessionRefs) {
    ...
    ipcMain.on('update-keybinds', (event, newKeybinds) => {
        if (!mainWindow.isDestroyed()) {
            updateGlobalShortcuts(newKeybinds, mainWindow, sendToRenderer, sessionRefs);
        }
    });
